...
"""
searchTypes = ['None', 'BFS', 'BFS_Turn', 'DFS', 'UCS', 'A*']
...
class VacuumPlanning(Problem):
    ...
    def generateSolution(self):
        ...
        if self.searchType == 'BFS':
            try:
                ...
                sol = path.solution()
                self.agent.performance -= path.path_cost
                self.env.set_solution(sol)
                self.env.display_explored(explored)
            except TypeError:
                print("No possible solution found with BFS. Stuck!")
            ...
            try:
                ...
                print("No possible solution found with DFS. Stuck!")
            ...
            try:
                ...
                print("No possible solution found with UCS. Stuck!")
            ...
            try:
                ...
                print("No possible solution found with A*. Stuck!")
            ...
            try:
                ...
                print("No possible solution found with BFS_Turn. Stuck!")
        else:
            raise 'NameError'
    ...
    def actions(self, state):
        ...
        for objects in possible_neighbors:
            if isinstance(objects[0], Wall):
                x,y = state
                if x + 1 == objects[0].location[0]:
                    possible_actions.remove('RIGHT')
                elif x - 1 == objects[0].location[0]:
                    possible_actions.remove('LEFT')
                elif y - 1 == objects[0].location[1]:
    ...
    def path_cost(self, c, state1, action, state2):
        ...
        if self.searchType == 'UCS' or self.searchType == 'A*':
            zstate1 = math.sqrt(state1[0]*state1[0] + state1[1]*state1[1])
            zstate2 = math.sqrt(state2[0]*state2[0] + state2[1]*state2[1])
            xstate = (state1[0] - state2[0])**2
            ystate = (state1[1] - state2[1])**2
            cost = xstate + ystate + ((zstate1-zstate2)**2)
            cost = math.sqrt(cost)
            cost += c
    ...
    """
        ...
        for dirts in self.env.things:
            if isinstance(dirts,Dirt):
                zDirtx, zDirty = dirts.location
                zAgt = math.sqrt(self.initial[0]*self.initial[0] + self.initial[1]*self.initial[1])
                zDirt = math.sqrt(zDirtx*zDirtx + zDirty*zDirty)
                minManHattan = manhattan_distance(node.state, dirts.location) + abs(zAgt - zDirt)
                h = minManHattan
        ...
        for dirts in self.env.things:
            if isinstance(dirts,Dirt):
                ...
                zAgt = math.sqrt(self.initial[0]*self.initial[0] + self.initial[1]*self.initial[1])
                zDirt = math.sqrt(zDirtx*zDirtx + zDirty*zDirty)
                currManHattan = manhattan_distance(node.state, dirts.location) + abs(zAgt - zDirt)
                
                if currManHattan < minManHattan:
                    minManHattan = currManHattan
                    h = minManHattan
...
class Gui(VacuumEnvironment):
    ...
    def setupTestEnvironment(self):
        ...
        if self.agent is not None:
            ...
            theAgent = XYSearchAgent(program=XYSearchAgentProgram, loc=(self.width//2, self.height//2))
        ...
        for _ in range(blockCount):
            ...
            if (colnum,rownum) == (self.height//2, self.width//2):
                while((colnum,rownum) == (self.height//2, self.width//2)):
                    rownum = random.choice(range(1, self.height - 1))
                    colnum = random.choice(range(1, self.width - 1))
    ...
    def display_explored(self, explored):
        ...
        if len(self.explored) > 0:     # means we have explored list from previous search. So need to clear their visual fist
            for (x, y) in self.explored:
                if not self.some_things_at((x,y),Wall) and x < self.width - 1 and x > 0 and y < self.height and y > 0:
                    self.buttons[y][x].config(bg='white')
        
        xi,yi = self.agent.location
        self.buttons[yi][xi].config(bg='light blue')

        self.explored = explored
        for (x, y) in explored:
            if not self.some_things_at((x,y),Wall) and x < self.width - 1 and x > 0 and y < self.height and y > 0 and self.buttons[y][x].cget('bg') != 'light blue':
            ...
            if action == 'RIGHT':
                if agent.direction.direction == Direction.U:
                    ...
                    if not agent.bump:
                        ...
                        if self.buttons[yf][xf].cget('bg') != 'grey':
                            self.buttons[yf][xf].config(bg='light blue')
    ...
    def update_env(self):
        ...
        if self.searchType == 'None' or self.searchType == None:
    ...
    def run(self, delay=2):
        ...
        while env.dirtCount > 0:
            ...
            if env.dirtCount == 0:
                ...
                self.done = True
                break
            elif self.searchType == 'None' or self.searchType == None:
                break
